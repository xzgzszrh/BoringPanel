...
export const SIMULATED_SSH_COMMANDS = [
    {
        id: "rolling-restart-quote-service",
        name: "滚动重启报价服务",
        description: "仅用于调试-报价服务 CPU 与线程池饱和场景；逐实例重启并等待健康检查通过。",
...
export function buildSimulatedSSHHost(updatedAt = Date.now()) {
    return {
...
export async function getDebugSimulationSettings(orgId) {
    const result = await db.execute({
        sql: "SELECT simulated_ssh_enabled, updated_at FROM scry_debug_simulation_settings WHERE org_id = ?",
        args: [orgId],
    });
...
export async function updateDebugSimulationSettings(orgId, simulatedSSHEnabled) {
...
export async function getSimulatedSSHHost(orgId) {
...
export async function isSimulatedSSHHost(hostId, orgId) {
    if (hostId !== SIMULATED_SSH_HOST_ID)
        return false;
