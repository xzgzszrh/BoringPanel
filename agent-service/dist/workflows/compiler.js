...
import { executeScryReadOperation } from '../tools/scry.js';
...
async function publishStep(writer, runId, step, status, detail) {
    await writer.custom({
...
function controlMarker(step, suffix) {
    return createStep({
...
function normalizeControlOutput(step, mode) {
    return createStep({
        ...
        execute: async ({ inputData, runId, writer }) => {
            ...
            if (!context)
                throw new Error(`控制节点“${step.name}”没有产生有效上下文`);
...
function prepareForeachStep(step) {
    return createStep({
        ...
        execute: async ({ inputData }) => {
            ...
            if (!Array.isArray(items))
                throw new Error(`foreach 数据路径“${step.loop?.sourcePath}”不是数组`);
...
function compileLeafStep(workflow, step, user, settings, threadId) {
            ...
            try {
                if (step.type === 'approval') {
                    ...
                    if (!decision.approved)
                        throw new Error(decision.comment || '用户拒绝了 Loop 审批');
                    const output = {
                        ...inputData,
                ...
                if (step.type === 'security-check') {
                    ...
                    if (result.decision === 'deny')
                        throw new Error(`安全决策拒绝执行：${result.reasons.join('；')}`);
                ...
                if (step.type === 'mcp-tool') {
                    ...
                    try {
                        ...
                        if (!tool?.execute)
                            throw new Error(`MCP 工具不存在或不可执行：${step.toolId || '未配置'}`);
...
function compileSequence(record, definitions, user, settings, id, threadId) {
    let workflow = createWorkflow({ id, inputSchema: contextSchema, outputSchema: contextSchema });
    for (const step of definitions) {
        if (step.type === 'condition' && step.condition) {
            ...
            workflow = workflow
                ...
                .branch([
                [async ({ inputData }) => compareRule(inputData, step.condition.rule), whenTrue],
                [async ({ inputData }) => !compareRule(inputData, step.condition.rule), whenFalse],
            ])
                .then(normalizeControlOutput(step, 'branch'));
            continue;
        }
        if (step.type === 'parallel' && step.parallel) {
            const branches = step.parallel.branches.map((branch) => compileSequence(record, branch.steps, user, settings, `${id}_${step.id}_${branch.id}`, threadId));
            workflow = workflow
                .then(controlMarker(step, 'start'))
                .parallel(branches)
        ...
        if (step.type === 'loop' && step.loop) {
            ...
            if (step.loop.mode === 'while') {
                workflow = workflow.dowhile(body, async ({ inputData, iterationCount }) => iterationCount < step.loop.maxIterations && compareRule(inputData, step.loop.rule));
            }
            else {
                workflow = workflow.dountil(body, async ({ inputData, iterationCount }) => iterationCount >= step.loop.maxIterations || compareRule(inputData, step.loop.rule));
            }
...
export function compileWorkflow(record, user, settings, threadId = '') {
    let workflow = compileSequence(record, record.definition.steps, user, settings, `${record.id}_loop`, threadId);
