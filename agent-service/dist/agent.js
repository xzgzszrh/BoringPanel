...
import { MCP_TOOL_CALL_PROTOCOL, SCRY_AGENT_TOOL_NAME_GUIDE, SCRY_QUERY_SIGNALS_PROTOCOL, } from "./mcp/protocol.js";
import { listEvidence } from "./evidence.js";
import { formatMemoryPacket, retrieveMemoryForContext, } from "./memory/index.js";
...
export async function createScryAgent(user, settings, mode = "diagnose", evidenceContext) {
    ...
    if (evidenceContext?.threadId && evidenceContext.query.trim()) {
        try {
            ...
            memoryPacket = formatMemoryPacket(await retrieveMemoryForContext(user, evidenceContext, evidence));
        }
        catch (error) {
