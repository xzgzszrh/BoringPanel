...
const POLICY_IDS = ["ssh-readonly-inspect", "ssh-execute-command"];
...
export async function listSSHHosts(enabledOnly = false, orgId = "") {
    const result = await db.execute(`SELECT * FROM scry_ssh_hosts ${enabledOnly ? "WHERE enabled = 1" : ""} ORDER BY name, created_at`);
...
export async function getSSHHost(id, orgId = "") {
    const simulated = orgId ? await getSimulatedSSHHost(orgId) : null;
    if (simulated?.id === id)
        return simulated;
...
export async function getSSHHostWithSecret(id, orgId = "") {
    const simulated = orgId ? await getSimulatedSSHHost(orgId) : null;
    if (simulated?.id === id)
        return simulated;
    const result = await db.execute({
    ...
    const secret = encrypted
        ? JSON.parse(decryptSecret(encrypted, config.masterKey))
        : {};
    return { ...hostFromRow(row), ...secret };
}
function validateSecret(authType, secret) {
...
function validateManagedUsername(username) {
...
export async function createSSHHost(input) {
    ...
    return (await getSSHHost(id));
}
export async function updateSSHHost(id, input) {
    validateManagedUsername(input.username);
    if (input.authType !== "private_key")
        throw new Error("scry-ops 账户只允许公钥认证");
...
function policyFromRow(row) {
    const data = JSON.parse(String(row.policy_json || "{}"));
    return {
...
export async function getSSHToolPolicy(toolId) {
    const result = await db.execute({
        sql: "SELECT * FROM scry_tool_policies WHERE tool_id = ?",
        args: [toolId],
    });
    if (!result.rows[0])
        throw new Error(`SSH 工具策略不存在：${toolId}`);
    return policyFromRow(result.rows[0]);
}
export async function listSSHToolPolicies() {
    const result = await db.execute({
...
export async function updateSSHToolPolicy(input) {
    const requireApproval = input.toolId === "ssh-execute-command" ? true : input.requireApproval;
    await db.execute({
      ...
      WHERE tool_id = ?`,
        args: [
            ...
            requireApproval ? 1 : 0,
            JSON.stringify({
                mode: input.toolId === "ssh-execute-command"
                    ? "configured-only"
                    : "builtin-only",
...
export async function addSSHAudit(input) {
    await db.execute({
...
export async function listSSHAudits(limit = 50) {
    const result = await db.execute({
