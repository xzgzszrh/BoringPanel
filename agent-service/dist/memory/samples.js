...
async function generatedSamples(user) {
    ...
    for (const row of result.rows) {
        ...
        if (sampleId && !generated.has(sampleId))
            generated.set(sampleId, String(row.id));
    }
    return generated;
}
function catalogItem(sample, generatedMemoryId = '') {
    const { content: _content, ...item } = sample;
    return { ...item, generatedMemoryId };
}
export async function listMemorySamples(user) {
...
export async function generateMemorySamples(input) {
    ...
    if (definitions.some((sample) => !sample))
        throw new Error('包含未知的示例记忆');
...
export async function deleteSampleMemory(memoryId, user) {
    const memory = await getMemory(memoryId, user);
    if (!memory)
        return null;
    if (!memory.tags.includes(SAMPLE_TAG))
        throw new Error('只有示例记忆可以使用彻底删除');
