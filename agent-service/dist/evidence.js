...
function serializeContent(value) {
    ...
    if (Buffer.byteLength(json) <= MAX_CONTENT_BYTES)
        return { content: sanitized, json, truncated: false };
...
function dataQuality(value, truncated) {
    if (truncated)
        return 68;
    if (value === null || value === undefined || value === '')
        return 30;
    if (Array.isArray(value))
        return value.length ? 92 : 48;
    if (typeof value === 'object')
        return Object.keys(value).length ? 90 : 48;
    return String(value).trim() ? 82 : 30;
}
function sourceCueMatch(query, sourceType, sourceName) {
    const cues = {
...
export function calculateEvidenceScores(input) {
    ...
    const confidence = clampScore(factors.sourceReliability * 0.34
...
function summaryFor(value) {
    if (Array.isArray(value))
        return value.length ? `返回 ${value.length} 项结构化记录。` : '查询成功，但未返回记录。';
    if (typeof value === 'string')
        return redactString(value).replace(/\s+/g, ' ').trim().slice(0, 180) || '返回空文本。';
    if (value && typeof value === 'object') {
        const record = value;
        const data = record.data;
        if (Array.isArray(data))
            return data.length ? `返回 ${data.length} 项结构化记录。` : '查询成功，但数据集合为空。';
...
function sourceTypeForTool(toolName) {
    const normalized = toolName.toLowerCase();
    if (normalized.includes('ssh'))
        return 'ssh';
    if (normalized.includes('query-signals') || normalized.includes('querysignals'))
        return 'telemetry';
    if (normalized.includes('list-services') || normalized.includes('listservices'))
        return 'service';
    if (normalized.includes('list-alerts') || normalized.includes('listalerts'))
        return 'alert';
    if (normalized.includes('list-dashboards') || normalized.includes('listdashboards'))
        return 'dashboard';
    return 'mcp';
}
...
function evidenceFromRow(row) {
    return {
        ...
        scoreFactors: JSON.parse(String(row.score_factors_json || '{}')),
...
async function hydrateEvidence(items) {
    if (!items.length)
        return items;
    ...
    return items.map((item) => ({
        ...item,
        relations: relations.filter((relation) => ids.has(relation.fromEvidenceId) && ids.has(relation.toEvidenceId)
            && (relation.fromEvidenceId === item.id || relation.toEvidenceId === item.id)),
        actions: actions.filter((action) => action.evidenceId === item.id),
    }));
}
async function createRelations(item) {
...
export async function recordEvidence(input) {
    ...
    const initialStatus = injection.detected ? 'suspended' : 'active';
...
export async function listEvidence(threadId, orgId) {
    const result = await db.execute({
...
export async function getEvidenceByIds(threadId, orgId, ids) {
    if (!ids.length)
        return [];
...
export async function updateEvidence(input) {
    ...
    if (input.status && input.status !== current.status)
        actions.push(input.status === 'active' ? 'restored' : input.status);
    if (input.selected !== undefined && selected !== current.selected)
        actions.push(selected ? 'selected' : 'unselected');
    if (input.note !== undefined && note !== current.note)
        actions.push('noted');
    await db.execute({
    ...
    if (updated && current.status !== 'accepted' && updated.status === 'accepted') {
        await learnMemoryFromEvidence(updated, input.user).catch((error) => console.error('Failed to learn memory from accepted evidence:', error));
    }
    return updated;
}
    ...
    for (const value of parts) {
        ...
        if (type === 'data-scry-workflow-step') {
            ...
            if (data.status !== 'completed' || data.detail === undefined
                || !['agent', 'verify', 'operation'].includes(String(data.stepType || '')))
                continue;
            await recordEvidence({
                user: input.user,
                context: input.context,
