...
import { z } from "zod";
import { isSimulatedSSHHost, SIMULATED_SSH_COMMANDS, } from "../debug/simulation.js";
import { addSSHAudit, getSSHHostWithSecret, getSSHToolPolicy, listSSHHosts, } from "../ssh/repository.js";
...
import { executeSSHCommand, SSH_READONLY_COMMAND_LABELS, SSH_READONLY_COMMANDS, } from "./ssh.js";
import { recordEvidence } from "../evidence.js";
async function auditedExecution(input) {
    const host = await getSSHHostWithSecret(input.hostId, input.user.orgId);
    if (!host || !host.enabled)
        throw new Error("SSH 主机不存在或已停用");
    ...
    try {
        const result = await executeSSHCommand(host, input.command, input.timeoutSeconds);
        await addSSHAudit({
            userId: input.user.id,
            hostId: host.id,
            hostName: host.name,
        ...
        if (input.evidenceContext) {
            ...
            }).catch((error) => console.error(`Failed to record SSH evidence ${input.commandId}:`, error));
        }
...
export function createSSHTools(user, evidenceContext) {
    const listHosts = createTool({
        id: "ssh-list-hosts",
        description: "列出管理员已经授权给 Agent 使用的 SSH 主机和可选择的命令 ID。调用 sshReadonlyInspect 或 sshExecuteCommand 前必须先调用本工具；结果不包含密码、私钥或精确命令文本。",
        ...
        outputSchema: z.object({
            ...
            configuredCommands: z.array(z.object({ id: z.string(), name: z.string(), description: z.string() })),
            a2ui: z.any(),
        }),
        execute: async () => {
            ...
            const readonlyCommands = readonlyCommandIds.map((id) => ({
                id,
                name: SSH_READONLY_COMMAND_LABELS[id] || id,
            }));
            const configuredDefinitions = [
                ...(commandPolicy.enabled ? commandPolicy.commands : []),
                ...(hasSimulatedHost ? SIMULATED_SSH_COMMANDS : []),
            ].filter((command, index, values) => values.findIndex((candidate) => candidate.id === command.id) === index);
            const configuredCommands = configuredDefinitions.map(({ id, name, description }) => ({
                id,
                name,
                description,
            }));
...
示例：{"hostId":"00000000-0000-4000-8000-000000000001","commandId":"disk_usage"}`,
        inputSchema: z
            ...
            hostId: z
                .string()
                .uuid()
                .describe("来自 sshListHosts 返回的 hosts[].id UUID，不是主机名或 IP。"),
            commandId: z
                .enum(Object.keys(SSH_READONLY_COMMANDS))
                .describe("来自 sshListHosts 返回的 readonlyCommands[].id。可选值分别表示系统概览、磁盘、内存、失败服务、监听端口、近期错误和安全预检。"),
        })
            .describe("直接传 hostId 和 commandId，不要传 hostname、command、args 或 input 包装层。"),
        outputSchema: z.object({
            hostName: z.string(),
        ...
        requireApproval: async ({ hostId }) => (await isSimulatedSSHHost(hostId, user.orgId)) ||
        ...
        execute: async ({ hostId, commandId }) => {
            ...
            return {
                ...result,
                commandName,
                a2ui: a2ui(`ssh-${hostId}-${commandId}`, "code", `${result.hostName} · ${commandName}`, {
                    stdout: result.stdout,
                    stderr: result.stderr,
                    exitCode: result.exitCode,
                    durationMs: result.durationMs,
...
示例：{"hostId":"00000000-0000-4000-8000-000000000001","commandId":"rolling-restart-quote-service"}`,
        inputSchema: z
            ...
            commandId: z
                .string()
                .regex(/^[a-zA-Z0-9_-]+$/)
                .describe("来自 sshListHosts 返回的 configuredCommands[].id；不得改写为命令文本。"),
        })
            .describe("直接传 hostId 和 commandId，不要传 hostname、command、args 或 input 包装层。"),
        outputSchema: z.object({
            hostName: z.string(),
        ...
        execute: async ({ hostId, commandId }) => {
            ...
            if (!definition)
                throw new Error("该 SSH 命令未获管理员授权");
            const result = await auditedExecution({
                user,
                hostId,
                commandId,
            ...
            return {
                ...
                a2ui: a2ui(`ssh-${hostId}-${commandId}`, "code", `${result.hostName} · ${definition.name}`, {
                    stdout: result.stdout,
                    stderr: result.stderr,
                    exitCode: result.exitCode,
                    durationMs: result.durationMs,
