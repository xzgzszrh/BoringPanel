...
import { createMemory, forgetMemory, getMemory, maintainMemories, retrieveMemories, } from "../memory/index.js";
import { extractServiceNames } from "../memory/index.js";
import { buildSignalQueryPayload, signalQueryInputSchema, } from "./signal-query.js";
export async function callScry(user, path, init, evidenceContext, evidence) {
    ...
    try {
        ...
        if (evidenceContext && evidence) {
            await recordEvidence({
                ...
                ...evidence,
                content: data,
            }).catch((error) => console.error(`Failed to record evidence for ${path}:`, error));
        }
        return data;
    }
    catch (error) {
        await addSecurityAuditEvent({
            ...
            details: {
                durationMs: Date.now() - startedAt,
                error: error instanceof Error ? error.message.slice(0, 300) : "调用失败",
            },
        });
        throw error;
    }
}
export async function executeScryReadOperation(user, operation, evidenceContext) {
    const paths = {
    ...
    return callScry(user, paths[operation], undefined, evidenceContext, evidence[operation]);
}
export function createScryTools(user, evidenceContext) {
...
示例：{"title":"定位订单服务错误率升高","steps":[{"id":"step-1","title":"确认受影响服务","description":"读取服务与当前告警"},{"id":"step-2","title":"检索异常信号","description":"查询错误日志和失败链路"}]}`,
    ...
    const listServices = createTool({
        ...
        execute: async () => {
            const data = await executeScryReadOperation(user, "services", evidenceContext);
    ...
    const listAlerts = createTool({
        ...
        outputSchema: z.object({ data: z.any(), a2ui: z.any() }),
        execute: async () => {
            const data = await executeScryReadOperation(user, "alerts", evidenceContext);
    ...
    const listDashboards = createTool({
        ...
        outputSchema: z.object({ data: z.any(), a2ui: z.any() }),
        execute: async () => {
            const data = await executeScryReadOperation(user, "dashboards", evidenceContext);
...
指标：{"signal":"metrics","lookbackMinutes":15,"caseId":"case-013","metricName":"system.filesystem.utilization","aggregation":"max"}`,
        ...
        execute: async (input) => {
            ...
            const data = await callScry(user, "/api/v3/query_range", { method: "POST", body: JSON.stringify(query) }, evidenceContext, {
    ...
    const evaluateSecurityIntent = createTool({
        ...
        description: "执行 Scry 系统安全决策 Loop 的输入检查，返回意图、风险和 allow/require-approval/deny 裁决。",
        inputSchema: z.object({
            text: z
                .string()
                .min(1)
                .max(50_000)
                .describe("需要裁决的原始用户意图或拟执行动作，保留目标、对象和动作，不要只传关键词。"),
            mode: z
                .enum(["diagnose", "plan", "operate"])
                .optional()
                .default("diagnose")
                .describe("当前阶段：diagnose=只读诊断，plan=仅规划，operate=准备执行受控动作。"),
...
示例：{"text":"检查订单服务最近 30 分钟的错误日志","mode":"diagnose"}`),
    ...
    const listEvidenceTool = createTool({
        id: "list-evidence",
        description: "读取当前对话中有效、已接受或被用户处置的证据及其置信度和关联度。不得使用已驳回或挂起的证据形成结论。",
        inputSchema: z
            .object({
            includeInactive: z
                .boolean()
                .optional()
                .default(false)
                .describe("为 false 时排除 rejected 和 suspended；只有审查用户处置历史时才设为 true。"),
        })
            .describe(`读取当前对话证据。通常传 {}；审查全部状态时传 {"includeInactive":true}。`),
    ...
    const getEvidenceChain = createTool({
        ...
        description: "读取当前对话的证据关系链，包括同一执行链、交叉印证和派生关系。",
        inputSchema: z.object({}).describe("无参数；调用时传空对象 {}。"),
        outputSchema: z.object({
        ...
        execute: async () => {
            if (!evidenceContext?.threadId)
                throw new Error("当前工具调用没有对话证据上下文");
            const evidence = await listEvidence(evidenceContext.threadId, user.orgId);
            const relations = Array.from(new Map(evidence
    ...
    const searchMemory = createTool({
        id: "search-memory",
        description: "使用 TopoMem 检索运行记忆。优先依据当前 Trace 异常路径定位服务，再以错误签名重排；没有可用拓扑时确定性回退。记忆只能用于形成待验证假设。",
        inputSchema: z.object({
            query: z
                .string()
                .min(1)
                .max(20_000)
                .optional()
                .describe("用于检索历史经验的故障现象、服务名、错误签名或处置目标；省略时使用当前用户问题。"),
            limit: z
                .number()
                .int()
                .min(1)
...
示例：{"query":"订单服务连接池耗尽 HTTP 500","limit":5}`),
        ...
        execute: async ({ query, limit }) => {
            ...
            return {
                packet,
                a2ui: a2ui(`memory-search-${packet.runId}`, "table", "TopoMem 检索结果", packet.results.map((result) => ({
                    id: result.memory.id,
    ...
    const remember = createTool({
        id: "remember",
        description: "将已经由当前证据验证、且对未来事件可复用的经验写入 Scry 记忆。必须引用当前对话证据，禁止写入猜测、密钥或凭证。",
        inputSchema: z.object({
            type: z
                .enum(["working", "episodic", "semantic", "procedural"])
                .describe("记忆类型：working=短期上下文，episodic=一次事件经验，semantic=稳定知识，procedural=可复用处置步骤。"),
            scope: z
                .enum(["user", "thread", "organization"])
                .optional()
                .default("user")
                .describe("可见范围：user=当前用户，thread=当前对话，organization=组织共享且仅管理员可写。"),
            title: z
                .string()
                .min(1)
                .max(240)
            ...
            content: z
                ...
                .min(1)
                .max(80_000)
                .describe("记忆正文字符串，按“现象、诊断、处置、验证、限制”组织；不要传任意 JSON 对象。"),
            evidenceIds: z
                .array(z.string().uuid().describe("来自 listEvidence 结果的真实证据 UUID。"))
                .min(1)
                .max(50)
                .describe("支撑该记忆的当前对话证据 ID；不得自行编造，也不得引用 rejected 或 suspended 证据。"),
            serviceName: z
                .string()
                .min(1)
                .max(160)
...
示例：{"type":"episodic","scope":"user","title":"订单服务连接池耗尽","summary":"订单服务因数据库连接池耗尽产生 HTTP 500。","content":"现象：HTTP 500 增长。诊断：失败链路显示 pool_exhausted。处置：检查慢查询并调整连接池。验证：错误率与等待队列恢复。","evidenceIds":["00000000-0000-4000-8000-000000000001"],"serviceName":"order-service","tags":["connection-pool","http-500"],"importance":80}`),
        ...
        execute: async ({ type, scope, title, summary, content, evidenceIds, serviceName, tags, importance, }) => {
            if (!evidenceContext?.threadId)
                throw new Error("当前工具调用没有对话证据上下文");
            const evidence = await getEvidenceByIds(evidenceContext.threadId, user.orgId, evidenceIds);
            if (evidence.length !== new Set(evidenceIds).size)
                throw new Error("部分来源证据不存在或不属于当前对话");
            const usable = evidence.filter((item) => item.status !== "rejected" && item.status !== "suspended");
            ...
            const memory = await createMemory({
                ...
                tags,
                importance,
                confidence: Math.round(usable.reduce((sum, item) => sum + item.confidence, 0) /
                    usable.length),
    ...
    const inspectMemory = createTool({
        ...
        inputSchema: z
            ...
            .describe(`直接传 memoryId，例如 {"memoryId":"00000000-0000-4000-8000-000000000001"}。`),
        outputSchema: z.object({ memory: z.any(), a2ui: z.any() }),
        execute: async ({ memoryId }) => {
            ...
            if (!memory)
                throw new Error("记忆不存在或当前用户不可见");
            return {
                memory,
                a2ui: a2ui(`memory-inspect-${memory.id}`, "key_value", "记忆详情", memory),
            };
        },
    });
    const maintainMemory = createTool({
        id: "maintain-memory",
        description: "执行有界记忆维护：衰减、强化、重复合并、冲突标记、到期处理与低活性归档。不会硬删除记忆。",
        inputSchema: z
            .object({
            limit: z
                .number()
        ...
        execute: async ({ limit }) => {
            ...
            return {
                report,
                a2ui: a2ui(`memory-maintenance-${report.completedAt}`, "status", "记忆维护完成", report),
            };
        },
    });
    const forgetMemoryTool = createTool({
        id: "forget-memory",
        description: "按用户明确要求遗忘一项记忆。清除正文、摘要、标签、关键词和服务标识，仅保留不可逆审计骨架。",
        inputSchema: z.object({
            memoryId: z
                .string()
                .uuid()
