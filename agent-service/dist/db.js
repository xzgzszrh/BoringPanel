...
import { createWasmSQLiteClient } from './database/wasm-sqlite.js';
import { workflowPresets } from './workflows/presets.js';
export const db = await createWasmSQLiteClient(config.databaseUrl);
...
function mcpServerFromRow(row, includeSecrets) {
    const encrypted = String(row.headers_encrypted || '');
    let headers = {};
    if (includeSecrets && encrypted) {
        try {
            headers = JSON.parse(decryptSecret(encrypted, config.masterKey));
        }
        catch {
            headers = {};
        }
...
export async function listMCPServers(orgId, enabledOnly = false, includeSecrets = false) {
...
export async function getMCPServer(id, orgId, includeSecrets = false) {
    const result = await db.execute({
...
export async function createMCPServerRecord(input) {
    ...
    return (await getMCPServer(id, input.orgId));
}
export async function updateMCPServerRecord(id, orgId, input) {
...
export async function deleteMCPServerRecord(id, orgId) {
    await db.execute({ sql: 'DELETE FROM scry_mcp_servers WHERE id = ? AND org_id = ?', args: [id, orgId] });
}
export async function getModelSettings(includeSecret = false) {
    ...
    return {
        ...
        endpointMode: String(row?.endpoint_mode || 'official'),
        ...
        toolChoice: String(row?.tool_choice || 'auto'),
...
export async function createThread(userId, orgId, title = '新对话') {
    const id = randomUUID();
    const now = Date.now();
    await db.execute({
...
export async function upsertUIMessage(threadId, message) {
...
export async function getThreadUIMessages(threadId) {
    const result = await db.execute({
        sql: 'SELECT message_json FROM scry_agent_ui_messages WHERE thread_id = ? ORDER BY created_at, id',
        args: [threadId],
    });
    return result.rows.map((row) => JSON.parse(String(row.message_json)));
}
export async function updateThreadTitleFromMessage(threadId, content) {
    const result = await db.execute({
        sql: 'SELECT title FROM scry_agent_threads WHERE id = ?',
        args: [threadId],
    });
    if (String(result.rows[0]?.title || '') !== '新对话')
        return;
...
function workflowFromRow(row) {
    ...
    const schedule = parse(row.schedule_json, {});
    return {
        id: String(row.id),
        name: String(row.name),
        description: String(row.description || ''),
        ...
        eventTriggers: parse(row.event_triggers_json, []),
        inputSchema: parse(row.input_schema_json, []),
        version: Number(row.version || 1),
        versions: parse(row.versions_json, []),
        createdAt: Number(row.created_at),
        updatedAt: Number(row.updated_at),
    };
}
export async function listWorkflows(enabledOnly = false) {
    const result = await db.execute(`SELECT * FROM scry_workflows ${enabledOnly ? 'WHERE enabled = 1' : ''} ORDER BY updated_at DESC`);
...
export async function updateWorkflowRecord(id, input) {
...
export async function deleteWorkflowRecord(id) {
...
export async function updateWorkflowRunRecord(id, patch) {
    const current = await db.execute({ sql: 'SELECT step_state_json FROM scry_workflow_runs WHERE id = ?', args: [id] });
    if (!current.rows.length)
        return;
    const stepState = JSON.parse(String(current.rows[0].step_state_json || '{}'));
    if (patch.stepId)
        stepState[patch.stepId] = patch.stepState;
    await db.execute({
...
function workflowRunFromRow(row) {
    return {
        ...
        stepState: JSON.parse(String(row.step_state_json || '{}')),
        suspendedStep: String(row.suspended_step || ''),
        createdAt: Number(row.created_at),
        updatedAt: Number(row.updated_at),
    };
}
export async function listWorkflowRuns(workflowId, userId) {
    const result = await db.execute({
