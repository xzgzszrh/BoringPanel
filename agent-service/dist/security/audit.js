...
export async function listSecurityAuditEvents(orgId, limit = 100) {
    ...
    return result.rows.map((row) => ({
        ...
        decision: String(row.decision), riskScore: Number(row.risk_score),
        inputHash: String(row.input_hash), details: JSON.parse(String(row.details_json || '{}')),
