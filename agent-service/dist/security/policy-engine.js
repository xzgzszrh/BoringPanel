...
export function securityInputHash(value) {
    return createHash("sha256")
        .update(typeof value === "string" ? value : JSON.stringify(value))
        .digest("hex");
}
export async function evaluateInputSecurity(context) {
...
export async function authorizeSSHExecution(input) {
    ...
    const root = input.host.username.trim().toLowerCase() === "root" ||
        input.host.username.trim() === "0";
    const decision = root || analysis.decision === "deny"
        ? "deny"
