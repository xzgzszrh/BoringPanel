...
app.post('/api/mcp/servers/:serverId/test', async (c) => {
  ...
  if (!server) return c.json({ error: 'MCP 服务不存在' }, 404);
...
app.post('/api/settings/model/test', async (c) => {
  ...
  try {
    ...
    try {
      ...
      const response: ModelTestResult = {
        ok: true,
...
const sshPolicyInputSchema = z.object({
  ...
  commandIds: z.array(z.enum(Object.keys(SSH_READONLY_COMMANDS) as [keyof typeof SSH_READONLY_COMMANDS, ...(keyof typeof SSH_READONLY_COMMANDS)[]])).max(20),
...
app.put('/api/debug/simulation', async (c) => {
  ...
  return c.json(await updateDebugSimulationSettings(
    currentUser(c).orgId,
    input.simulatedSSHEnabled,
  ));
});
...
app.put('/api/ssh/hosts/:hostId', async (c) => {
  ...
  if (current?.simulated) return c.json({ error: '模拟 SSH 主机只能在调试模式中管理' }, 409);
  const input = sshHostInputSchema.parse(await c.req.json());
...
app.delete('/api/ssh/hosts/:hostId', async (c) => {
  ...
  if (denied) return denied;
  const current = await getSSHHost(c.req.param('hostId'), currentUser(c).orgId);
  if (current?.simulated) return c.json({ error: '请在调试模式中关闭模拟 SSH 主机' }, 409);
...
app.post('/api/ssh/hosts/:hostId/test', async (c) => {
  ...
  if (!host) return c.json({ error: 'SSH 主机不存在' }, 404);
  const startedAt = Date.now();
  try {
...
app.put('/api/ssh/policies/:toolId', async (c) => {
  ...
  if (!current) return c.json({ error: 'SSH 工具策略不存在' }, 404);
...
app.get('/api/threads/:threadId', async (c) => {
  ...
  if (!thread.rows.length) return c.json({ error: '对话不存在' }, 404);
...
app.delete('/api/threads/:threadId', async (c) => {
  ...
  if (!owned.rows.length) return c.body(null, 204);
  await db.batch([
...
function messageText(message: z.infer<typeof uiMessageSchema>): string {
...
function isTextAttachment(part: Record<string, unknown>): boolean {
...
function decodeDataAttachment(part: Record<string, unknown>): Buffer {
  const url = String(part.url || '');
  const separator = url.indexOf(',');
  if (!url.startsWith('data:') || separator < 0) throw new Error('附件必须使用内嵌 data URL');
...
function normalizeModelAttachments(
  messages: Array<z.infer<typeof uiMessageSchema>>,
): Array<z.infer<typeof uiMessageSchema>> {
  return messages.map((message) => {
    const fileParts = message.parts.filter((part) => part.type === 'file');
    if (fileParts.length > 5) throw new Error('每条消息最多包含 5 个附件');
    return {
      ...message,
      parts: message.parts.flatMap((part) => {
        if (part.type !== 'file') return [part];
        const content = decodeDataAttachment(part);
        if (content.byteLength > FILE_ATTACHMENT_MAX_BYTES) throw new Error('单个附件不能超过 10 MB');
        if (!isTextAttachment(part)) return [part];
        if (content.byteLength > TEXT_ATTACHMENT_MAX_BYTES) throw new Error('文本附件不能超过 512 KB');
...
async function ownsThread(threadId: string, userId: string, orgId: string): Promise<boolean> {
  const result = await db.execute({
    sql: 'SELECT id FROM scry_agent_threads WHERE id = ? AND user_id = ? AND org_id = ?',
...
app.get('/api/memories', async (c) => {
  ...
  return c.json(await listMemories({
    user, query: input.query, type: input.type as MemoryType | undefined, scope: input.scope as MemoryScope | undefined,
    status: input.status as MemoryStatus | undefined, serviceName: input.serviceName,
...
app.post('/api/memories', async (c) => {
  ...
  if (input.threadId && !await ownsThread(input.threadId, user.id, user.orgId)) return c.json({ error: '对话不存在' }, 404);
  const evidence = input.threadId && input.evidenceIds.length
    ? await getEvidenceByIds(input.threadId, user.orgId, input.evidenceIds) : [];
  if (evidence.length !== input.evidenceIds.length) return c.json({ error: '部分来源证据不存在或不属于当前对话' }, 400);
  if (evidence.some((item) => item.status === 'rejected' || item.status === 'suspended')) {
    return c.json({ error: '已驳回或挂起的证据不能作为记忆来源' }, 409);
  }
  const memory = await createMemory({
    user, ...input, content: input.content ?? null, type: input.type as MemoryType, scope: input.scope as MemoryScope,
    sources: evidence.map((item) => ({ sourceType: 'evidence' as const, sourceId: item.id, traceId: item.traceId,
...
app.patch('/api/memories/:memoryId', async (c) => {
  ...
  const memory = await updateMemory({ id: c.req.param('memoryId'), user: currentUser(c),
    ...input, status: input.status as MemoryStatus | undefined });
  return memory ? c.json(memory) : c.json({ error: '记忆不存在' }, 404);
});
...
app.get('/api/threads/:threadId/memories', async (c) => {
  const user = currentUser(c);
  const threadId = c.req.param('threadId');
  if (!await ownsThread(threadId, user.id, user.orgId)) return c.json({ error: '对话不存在' }, 404);
  const messages = await getThreadUIMessages(threadId);
  const latestUser = [...messages].reverse().find((message) => message.role === 'user');
  const query = latestUser && Array.isArray(latestUser.parts)
    ? latestUser.parts.filter((part) => part && typeof part === 'object' && (part as Record<string, unknown>).type === 'text')
  ...
  if (!evidence.length) {
    ...
    for (const message of messages) {
      ...
      if (message.role === 'user') {
        query = parts.filter((part) => part && typeof part === 'object' && (part as Record<string, unknown>).type === 'text')
          .map((part) => String((part as Record<string, unknown>).text || '')).join('\n').trim();
        continue;
      }
...
app.patch('/api/evidence/:evidenceId', async (c) => {
  ...
  const evidence = await updateEvidence({
    ...
    user,
    status: input.status as EvidenceStatus | undefined,
...
app.post('/api/threads/:threadId/evidence/revise', async (c) => {
  ...
  if (selected.length !== new Set(input.evidenceIds).size) return c.json({ error: '部分证据不存在或不属于当前对话' }, 400);
  const usable = selected.filter((item) => item.status !== 'rejected' && item.status !== 'suspended');
  if (!usable.length) return c.json({ error: '选定证据均已被驳回或挂起' }, 409);
  ...
  const previousAnswer = latestAssistant && Array.isArray(latestAssistant.parts)
    ? latestAssistant.parts.filter((part) => part && typeof part === 'object' && (part as Record<string, unknown>).type === 'text')
      .map((part) => String((part as Record<string, unknown>).text || '')).join('\n').trim()
    : '';
  ...
  const excludedText = excluded.map((item) =>
    `${item.id} ${item.title}（${item.status === 'rejected' ? '已驳回' : '已挂起'}）${item.note ? `：${item.note}` : ''}`)
    .join('\n').slice(0, 20_000);
...
async function persistAssistantUIStream(
  ...
  for await (const message of readUIMessageStream<UIMessage>({ stream })) finalMessage = message;
...
app.post('/api/chat', async (c) => {
  ...
  if (!await ownsThread(input.threadId, user.id, user.orgId)) return c.json({ error: '对话不存在' }, 404);
  let modelMessages: Array<z.infer<typeof uiMessageSchema>>;
  try {
  ...
  const evidenceContext: EvidenceCaptureContext = {
  ...
  if (!input.resumeData) {
    ...
    if (!latest) return c.json({ error: '缺少用户消息' }, 400);
    const text = messageText(latest);
    const hasFiles = latest.parts.some((part) => part.type === 'file');
    if (!text && !hasFiles) return c.json({ error: '消息内容不能为空' }, 400);
  ...
  try {
    stream = await handleChatStream<UIMessage>({
      mastra,
      agentId: agent.id,
      params: {
        messages: modelMessages as unknown as UIMessage[],
        runId: input.runId,
  ...
  void persistAssistantUIStream(persistenceStream, input.threadId, user, evidenceContext).catch((error) =>
    console.error('Failed to persist assistant UI stream', sanitizeProviderMessage(String(error))))
    .finally(disconnect);
...
const workflowStepSchema: z.ZodType<WorkflowStepDefinition> = z.lazy(() => z.object({
  id: workflowIdSchema,
  ...
  loop: z.object({
    ...
    if (loop.mode === 'foreach' && !loop.sourcePath) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['sourcePath'], message: 'foreach 循环需要数据源路径' });
  }).optional(),
}).superRefine((step, ctx) => {
  if (step.type === 'condition' && !step.condition) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['condition'], message: '条件节点缺少分支定义' });
  if (step.type === 'parallel' && !step.parallel) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['parallel'], message: '并行节点缺少分支定义' });
  if (step.type === 'loop' && !step.loop) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['loop'], message: '循环节点缺少循环定义' });
  if (step.type === 'mcp-tool' && !step.toolId) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['toolId'], message: 'MCP 工具节点缺少工具 ID' });
}));

function inspectWorkflowSteps(steps: WorkflowStepDefinition[]): { count: number; depth: number; ids: string[] } {
  let count = 0;
  let depth = 1;
  const ids: string[] = [];
  const visit = (items: WorkflowStepDefinition[], currentDepth: number): void => {
    ...
    items.forEach((step) => {
      ...
      if (step.loop) visit(step.loop.steps, currentDepth + 1);
    });
  };
  visit(steps, 1);
...
const workflowInputSchema = z.object({
  ...
  definition: z.object({
    ...
    if (inspection.count > 100) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['steps'], message: 'Loop 最多包含 100 个节点' });
    if (inspection.depth > 5) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['steps'], message: '控制节点最多嵌套 5 层' });
    if (new Set(inspection.ids).size !== inspection.ids.length) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['steps'], message: '所有节点 ID 必须唯一' });
  }),
});
...
app.post('/api/workflows', async (c) => {
  ...
  return c.json(await createWorkflowRecord({ ...input, definition: input.definition as WorkflowDefinition,
    schedule: input.schedule as WorkflowScheduleConfig, eventTriggers: input.eventTriggers as WorkflowEventTrigger[],
    inputSchema: input.inputSchema as WorkflowInputField[] }), 201);
});

app.put('/api/workflows/:workflowId', async (c) => {
  const denied = requireAdmin(c);
  if (denied) return denied;
  if (c.req.param('workflowId').startsWith('system-')) return c.json({ error: '系统 Loop 已锁定，不能修改' }, 403);
  const input = workflowInputSchema.parse(await c.req.json());
  const updated = await updateWorkflowRecord(c.req.param('workflowId'), { ...input, definition: input.definition as WorkflowDefinition,
    schedule: input.schedule as WorkflowScheduleConfig, eventTriggers: input.eventTriggers as WorkflowEventTrigger[],
    inputSchema: input.inputSchema as WorkflowInputField[] });
...
app.delete('/api/workflows/:workflowId', async (c) => {
  const denied = requireAdmin(c);
  if (denied) return denied;
  if (c.req.param('workflowId').startsWith('system-')) return c.json({ error: '系统 Loop 已锁定，不能删除' }, 403);
  if (!await getWorkflow(c.req.param('workflowId'))) return c.body(null, 204);
...
app.post('/api/workflows/:workflowId/stream', async (c) => {
  const user = currentUser(c);
  const workflow = await getWorkflow(c.req.param('workflowId'));
  if (!workflow || !workflow.enabled) return c.json({ error: 'Loop 不存在或已停用' }, 404);
  const input = z.object({
    threadId: z.string().min(1),
  ...
  void persistAssistantUIStream(persistenceStream, input.threadId, user, evidenceContext, `${runId}-result`).catch((error) =>
    console.error('Failed to persist Loop UI stream', sanitizeProviderMessage(String(error))));
  return createUIMessageStreamResponse({ stream: clientStream });
});
...
app.post('/api/workflow-runs/:runId/resume', async (c) => {
  ...
  void persistAssistantUIStream(persistenceStream, run.threadId, user, evidenceContext, `${run.id}-result`).catch((error) =>
    console.error('Failed to persist resumed Loop UI stream', sanitizeProviderMessage(String(error))));
  return createUIMessageStreamResponse({ stream: clientStream });
});

function safeEndpoint(settings: ModelSettings): string {
  try {
...
function diagnoseModelError(error: unknown): Pick<ModelTestResult, 'category' | 'message'> {
  ...
  if (error instanceof Error && error.name === 'AbortError') return { category: 'timeout', message: '请求超时，请检查端点可达性或增大超时时间' };
  if (/自定义端点|Invalid URL|API Key/.test(raw) && !status) return { category: 'configuration', message: raw };
  if (status === 401 || status === 403 || /api.?key|unauthor|authenticat|credential/i.test(raw)) return { category: 'authentication', message: 'API Key 被目标端点拒绝。请确认密钥属于当前端点；若使用兼容服务，请切换到“兼容端点”并填写其 Base URL。' };
  if (status === 404 || /not found|unknown endpoint/i.test(raw)) return { category: 'provider', message: '目标端点未提供所需接口。OpenAI 兼容服务必须实现 Responses API。' };
  if (status === 429 || /rate.?limit|quota/i.test(raw)) return { category: 'rate_limit', message: '模型服务返回限流或额度不足，请稍后重试或检查账户额度。' };
  if (/model.*not|unknown model|does not exist/i.test(raw)) return { category: 'model', message: '目标端点不支持当前模型 ID，请检查模型名称。' };
  if (/fetch failed|ECONN|ENOTFOUND|network|socket/i.test(raw)) return { category: 'network', message: '无法连接目标端点，请检查 Base URL、代理和网络连通性。' };
  return { category: 'provider', message: raw || '模型服务返回未知错误' };
}

app.onError((error, c) => {
  console.error(error);
  if (error instanceof z.ZodError) return c.json({ error: '请求参数无效', details: error.issues }, 400);
