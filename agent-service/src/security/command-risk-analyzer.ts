...
export function analyzeCommandRisk(command: string): CommandRiskResult {
  const reasons: string[] = [];
  const trimmed = command.trim();
  const tokens = tokenize(trimmed);
  const executable = (tokens[0] || '').split('/').pop()?.toLowerCase() || '';
  if (!trimmed || !tokens.length) reasons.push('命令为空或引号结构不完整');
  if (CONTROL_OPERATORS.test(trimmed)) reasons.push('命令包含 Shell 控制、替换或重定向操作符');
  const safeSudoWrapper = executable === 'sudo'
    && tokens[1] === '-n'
    && /^\/opt\/scry-ops\/bin\/scry-[a-z0-9-]+$/.test(tokens[2] || '');
  if (FORBIDDEN_EXECUTABLES.has(executable) && !safeSudoWrapper) reasons.push(`禁止通过 ${executable || 'Shell'} 解释器或提权程序执行`);
  if (DESTRUCTIVE.test(trimmed)) reasons.push('命令包含破坏性系统操作');
  if (tokens.some((token) => token.includes('\0'))) reasons.push('命令包含空字节');
  if (reasons.length) return { decision: 'deny', riskScore: 100, executable, tokens, reasons };

  const wrapper = /^\/opt\/scry-ops\/bin\/scry-[a-z0-9-]+$/.test(tokens[0] || '') || safeSudoWrapper;
  if (wrapper) return { decision: 'require-approval', riskScore: 65, executable, tokens, reasons: ['调用受管 scry-ops 包装器，执行前必须审批'] };
