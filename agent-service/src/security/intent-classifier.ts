...
export function classifyIntent(text: string): IntentClassification {
  if (PRIVILEGE.test(text)) return { intent: 'privilege-escalation', confidence: 0.98, requestedChange: true, reasons: ['请求包含权限提升或 root Shell 意图'] };
  if (SECRET.test(text)) return { intent: 'secret-access', confidence: 0.95, requestedChange: false, reasons: ['请求尝试读取认证凭证或密钥'] };
  if (CHANGE.test(text)) return { intent: 'change', confidence: 0.86, requestedChange: true, reasons: ['请求包含会改变系统状态的操作'] };
  if (DIAGNOSE.test(text)) return { intent: 'diagnose', confidence: 0.82, requestedChange: false, reasons: ['请求目标是故障诊断或根因定位'] };
  if (OBSERVE.test(text)) return { intent: 'observe', confidence: 0.8, requestedChange: false, reasons: ['请求目标是只读查询或巡检'] };
