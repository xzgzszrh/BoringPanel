...
function sanitizeValue(value: unknown, depth = 0): unknown {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, 300)
...
function serializeContent(value: unknown): { content: unknown; json: string } {
  const content = sanitizeValue(value);
  let json = JSON.stringify(stableValue(content));
  if (Buffer.byteLength(json) <= MAX_CONTENT_BYTES) return { content, json };
...
export function memoryTokens(value: string): Set<string> {
...
function hashedVector(value: string): Float64Array {
  ...
  for (let index = 0; index < normalized.length - 2; index += 1) {
    ...
    if (!/\s{2}/.test(gram)) features.push(gram);
  }
...
function denseSimilarity(left: string, right: string): number {
...
function extractKeywords(value: string, limit = 24): string[] {
...
function chunkType(title: string, content: string): MemoryChunkRecord['type'] {
  const value = `${title} ${content}`;
  if (/步骤|操作|处置|恢复|修复|runbook|procedure|remediation|command/i.test(value)) return 'procedure';
  if (/证据|日志|指标|链路|告警|evidence|trace|metric|log|alert/i.test(value)) return 'evidence';
  if (/观察|现象|相关|假设|observation|correlation|hypothesis/i.test(value)) return 'observation';
  if (/metadata|元数据|标识|配置|profile|属性/i.test(value)) return 'metadata';
  return 'summary';
}

function splitText(value: string, maxLength = 1800): string[] {
  ...
  const paragraphs = normalized.split(/\n{2,}/).flatMap((paragraph) => {
    ...
    if (sentences.length <= 1) {
      const slices: string[] = [];
      for (let index = 0; index < paragraph.length; index += maxLength) slices.push(paragraph.slice(index, index + maxLength));
      return slices;
    }
    return sentences;
  });
  for (const paragraph of paragraphs) {
    ...
    if (paragraph.length <= maxLength) buffer = paragraph;
    else {
      for (let index = 0; index < paragraph.length; index += maxLength) output.push(paragraph.slice(index, index + maxLength));
      buffer = '';
    }
  }
  if (buffer) output.push(buffer);
...
function buildMemoryChunks(input: {
  ...
  if (input.summary.trim()) sections.push({ title: '记忆摘要', content: input.summary.trim() });
  if (typeof input.content === 'string') {
    ...
    for (const [key, value] of Object.entries(input.content as Record<string, unknown>).slice(0, 80)) {
      const content = typeof value === 'string' ? value : JSON.stringify(value, null, 2);
      if (content && content !== 'null' && content !== '[]' && content !== '{}') sections.push({ title: key, content });
    }
...
function chunkFromRow(row: Record<string, unknown>): MemoryChunkRecord {
  return {
    id: String(row.id), memoryId: String(row.memory_id), index: Number(row.chunk_index),
    type: String(row.chunk_type) as MemoryChunkRecord['type'], title: String(row.title), content: String(row.content_text),
...
function objectString(record: Record<string, unknown>, keys: string[]): string {
  for (const key of keys) {
    const value = key.includes('.')
      ? key.split('.').reduce<unknown>((current, segment) => current && typeof current === 'object'
        ? (current as Record<string, unknown>)[segment] : undefined, record)
      : record[key];
...
function walkObjects(value: unknown, visitor: (record: Record<string, unknown>) => void, depth = 0): void {
  if (depth > 9 || !value || typeof value !== 'object') return;
...
export function extractServiceNames(value: unknown): string[] {
  ...
  walkObjects(value, (record) => {
    ...
    if (direct && direct.length <= 160) services.add(direct);
    for (const key of [...SERVICE_KEYS, ...FROM_SERVICE_KEYS, ...TO_SERVICE_KEYS]) {
      const candidate = objectString(record, [key]);
      if (candidate && candidate.length <= 160) services.add(candidate);
    }
  });
...
export function extractTraceEdges(value: unknown): TraceEdge[] {
  const objects: Record<string, unknown>[] = [];
  walkObjects(value, (record) => objects.push(record));
  const spans = new Map<string, { service: string; parentId: string; errorRate: number; calls: number }>();
  for (const record of objects) {
    const spanId = objectString(record, ['spanId', 'span_id', 'id']);
    const service = objectString(record, SERVICE_KEYS);
    if (!spanId || !service) continue;
  ...
  const add = (from: string, to: string, errorRate: number, calls: number): void => {
    if (!from || !to || from === to || from.length > 160 || to.length > 160) return;
    const key = `${from}\u0000${to}`;
  ...
  for (const span of spans.values()) {
    ...
    if (parent) add(parent.service, span.service, span.errorRate, span.calls);
  }
  for (const record of objects) {
...
export function extractErrorSignature(query: string, evidence: EvidenceItem[] = []): string[] {
...
function textForMemory(item: MemoryItem): string {
...
function contradictionScore(left: string, right: string): number {
  ...
  for (const [positive, negative] of pairs) {
    if ((positive.test(left) && negative.test(right)) || (negative.test(left) && positive.test(right))) score += 0.5;
  }
  return Math.min(1, score);
}

function memoryFromRow(row: Record<string, unknown>): MemoryItem {
  return {
    ...
    type: String(row.memory_type) as MemoryType, scope: String(row.scope) as MemoryScope,
    ...
    status: String(row.status) as MemoryStatus, pinned: Boolean(Number(row.pinned)), note: String(row.note || ''),
...
async function hydrateMemories(items: MemoryItem[]): Promise<MemoryItem[]> {
  ...
  const sources = sourceRows.rows.map((row): MemorySourceRecord => ({
    id: Number(row.id), memoryId: String(row.memory_id), sourceType: String(row.source_type) as MemorySourceType,
  ...
  const relations = relationRows.rows.map((row): MemoryRelationRecord => ({
    id: String(row.id), fromMemoryId: String(row.from_memory_id), toMemoryId: String(row.to_memory_id),
    type: String(row.relation_type) as MemoryRelationType, score: Number(row.score),
    rationale: String(row.rationale || ''), createdAt: Number(row.created_at),
  }));
  const actions = actionRows.rows.map((row): MemoryActionRecord => ({
    id: Number(row.id), memoryId: String(row.memory_id), userId: String(row.user_id), action: String(row.action),
    previousStatus: String(row.previous_status) as MemoryStatus, nextStatus: String(row.next_status) as MemoryStatus,
...
function isVisible(item: MemoryItem, user: AuthenticatedUser, context: MemorySearchContext = {}): boolean {
  if (item.orgId !== user.orgId) return false;
  if (item.scope === 'organization') return true;
  if (item.scope === 'thread') return Boolean(context.threadId && item.threadId === context.threadId);
...
export async function getMemory(id: string, user: AuthenticatedUser, context: MemorySearchContext = {}): Promise<MemoryItem | null> {
  ...
  if (!isVisible(item, user, context) && user.role !== 'ADMIN') return null;
...
async function addMemorySources(memoryId: string, sources: MemorySourceInput[], now = Date.now()): Promise<void> {
...
async function addMemoryAction(input: {
  memoryId: string; user: AuthenticatedUser; action: string; previousStatus: MemoryStatus; nextStatus: MemoryStatus; detail?: string;
}): Promise<void> {
  await db.execute({
...
async function createMemoryRelations(item: MemoryItem): Promise<void> {
  const rows = await db.execute({
  ...
  for (const row of rows.rows) {
    ...
    let type: MemoryRelationType | null = null;
    let score = 0;
    let rationale = '';
    if (item.contentHash === other.contentHash || similarity >= 0.86) {
      type = 'duplicates'; score = clampScore(Math.max(similarity, 0.9) * 100); rationale = '两项记忆包含近似相同的可复用经验。';
    } else if (contradiction >= 0.5 && similarity >= 0.12) {
      type = 'conflicts'; score = clampScore((contradiction * 0.7 + similarity * 0.3) * 100); rationale = '两项记忆对同一状态给出相反描述，需要重新验证。';
    } else if (item.serviceName && item.serviceName === other.serviceName && similarity >= 0.34) {
      type = 'supports'; score = clampScore(similarity * 100); rationale = '同一服务的独立记忆包含相互支持的状态或处置经验。';
    } else if (item.serviceName && item.serviceName === other.serviceName) {
      type = 'same_service'; score = 100; rationale = '两项记忆归属于同一运行服务。';
    }
    if (!type) continue;
    await db.execute({
...
export async function createMemory(input: {
  ...
  if (input.scope === 'organization' && input.user.role !== 'ADMIN') throw new Error('只有管理员可以写入组织级记忆');
  if (input.scope === 'thread' && !input.threadId) throw new Error('线程级记忆必须指定 threadId');
  if (input.scope === 'workflow' && !input.workflowId) throw new Error('Loop 级记忆必须指定 workflowId');
  ...
  const status: MemoryStatus = injection.detected ? 'suspended' : (input.status || 'active');
  ...
  if (existingRows.rows[0]) {
    ...
    return (await getMemory(existing.id, input.user, { threadId: input.threadId, workflowId: input.workflowId })) as MemoryItem;
  }
  const id = randomUUID();
  ...
  const created = (await getMemory(id, input.user, { threadId: input.threadId, workflowId: input.workflowId })) as MemoryItem;
  ...
  return (await getMemory(id, input.user, { threadId: input.threadId, workflowId: input.workflowId })) as MemoryItem;
}
...
export async function updateMemory(input: {
  ...
  if (current.scope === 'organization' && input.user.role !== 'ADMIN') throw new Error('只有管理员可以维护组织级记忆');
...
export async function forgetMemory(id: string, user: AuthenticatedUser): Promise<MemoryItem | null> {
  const current = await getMemory(id, user);
  if (!current) return null;
  if (current.scope === 'organization' && user.role !== 'ADMIN') throw new Error('只有管理员可以遗忘组织级记忆');
  await db.execute({
...
async function observeTopology(orgId: string, traceId: string, edges: TraceEdge[]): Promise<void> {
  const now = Date.now();
...
function enumeratePaths(edges: TraceEdge[], symptomService: string): string[][] {
  ...
  const walk = (service: string, path: string[], weight: number): void => {
    if (path.length >= 6) {
...
function chooseSymptomService(query: string, evidence: EvidenceItem[], edges: TraceEdge[]): string {
  ...
  for (const item of evidence.filter((candidate) => candidate.sourceType === 'alert' || candidate.sourceType === 'service')) {
    ...
    if (itemServices[0]) return itemServices[0];
  }
  return edges[0]?.from || '';
}

function topologyScore(serviceName: string, paths: string[][]): { score: number; path: string[] } {
  let best = 0;
  let matched: string[] = [];
  for (const path of paths) {
    ...
    if (score > best) { best = score; matched = path; }
  }
  return { score: clampScore(best), path: matched };
}

function signatureScore(itemText: string, signature: string[]): { score: number; matches: string[] } {
  if (!signature.length) return { score: 0, matches: [] };
...
export async function retrieveMemories(input: {
  ...
  if (edges.length) await observeTopology(input.user.orgId, input.context?.traceId || '', edges);
  ...
  let strategy: MemoryRetrievalPacket['strategy'] = allPaths.length > 1 ? 'topomem-mp' : allPaths.length === 1 ? 'topomem' : 'lexical-fallback';
  ...
  if (allPaths.length === 1) {
    ...
    if (!terminalMemories.length) {
      ...
      const support = terminalMemories.reduce((best, item) => Math.max(best,
        signatureScore(textForMemory(item), signature).score,
        clampScore(denseSimilarity(query, textForMemory(item)) * 100)), 0);
      if (support < 5) {
        strategy = 'lexical-fallback';
        paths = [];
  ...
  if (allPaths.length > 1) {
    ...
    if (support[0].value > 0 && support[0].value > (support[1]?.value || 0)) paths = [support[0].path];
  ...
  const scored: MemorySearchResult[] = candidates.map((memory) => {
    const chunkScores = (memory.chunks.length ? memory.chunks : [{
      id: memory.id, memoryId: memory.id, index: 0, type: 'summary' as const, title: memory.title,
    ...
    const factors: MemoryScoreFactors = { lexical, dense, topology: topology.score, signature: sig.score, quality, final };
    return {
      memory, score: final, factors, matchedPath: topology.path, signatureMatches: sig.matches,
      matchedChunks: chunkScores.slice(0, 3).map(({ chunk, lexical: chunkLexical, dense: chunkDense,
        signature: chunkSignature, score }) => ({ chunk, lexical: chunkLexical, dense: chunkDense,
...
export function formatMemoryPacket(packet: MemoryRetrievalPacket): string {
  if (!packet.results.length) return '';
...
function memoryTypeForEvidence(item: EvidenceItem): MemoryType {
  if (item.sourceType === 'document' && /手册|runbook|sop|步骤|操作/i.test(`${item.title} ${item.summary}`)) return 'procedural';
  if (item.sourceType === 'document' || item.sourceType === 'service' || item.sourceType === 'dashboard') return 'semantic';
  return 'episodic';
}

export async function learnMemoryFromEvidence(item: EvidenceItem, user: AuthenticatedUser): Promise<MemoryItem | null> {
...
export async function learnMemoryFromWorkflowOutcome(input: {
  ...
  if (!input.text.trim() || !usable.length) return null;
  ...
  return createMemory({
    ...
    sources: [
      ...
      ...usable.slice(0, 50).map((item): MemorySourceInput => ({
...
export async function maintainMemories(user: AuthenticatedUser, limit = 500): Promise<MemoryMaintenanceReport> {
  ...
  const report: MemoryMaintenanceReport = { scanned: items.length, decayed: 0, reinforced: 0, merged: 0,
  ...
  for (const item of items) {
    ...
    if (item.pinned || item.status === 'verified') continue;
  ...
  for (let leftIndex = 0; leftIndex < active.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < active.length; rightIndex += 1) {
      ...
      if (left.type !== right.type || (left.serviceName && right.serviceName && left.serviceName !== right.serviceName)) continue;
...
export async function memoryStats(user: AuthenticatedUser): Promise<{
  total: number; online: number; verified: number; suspended: number; averageVitality: number;
  byType: Record<MemoryType, number>; byStatus: Record<MemoryStatus, number>;
}> {
  const items = await listMemories({ user, includeInactive: true, limit: 500 });
  const byType: Record<MemoryType, number> = { working: 0, episodic: 0, semantic: 0, procedural: 0 };
  const byStatus: Record<MemoryStatus, number> = { active: 0, verified: 0, suspended: 0, archived: 0, forgotten: 0 };
