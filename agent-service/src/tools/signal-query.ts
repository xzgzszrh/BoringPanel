...
const signalFilterSchema = z
  .object({
    field: z
      ...
      .max(240)
      .describe(
        "字段名，例如 service.name、body、http.response.status_code 或 scry.debug.case_id。",
      ),
    ...
    scope: attributeScopeSchema
      ...
      .default("auto")
      .describe(
        "字段位置。标准列用 column，资源属性用 resource，Span/Log/Metric 属性用 attribute；通常使用 auto。",
      ),
  ...
  .superRefine((filter, context) => {
    if (
      !["exists", "nexists"].includes(filter.operator) &&
  ...
  .describe(
    "单个附加过滤条件。所有字段直接放在该对象中，通常使用 scope=auto。",
  );
...
export const signalQueryInputSchema = z
  .object({
    ...
    caseId: z
      ...
      .optional()
      .describe(
        "可选的不透明调试案例编号，例如 case-013。工具会转换为资源属性过滤。",
      ),
    serviceNames: z
      ...
      .default([])
      .describe(
        "可选服务名列表。多个服务使用一次 in 过滤，不要自行构造 OR 查询。",
      ),
    filters: z
    ...
    resultMode: z
      ...
      .default("records")
      .describe(
        "日志和链路默认返回 records 原始记录；需要数量趋势时使用 timeseries。指标始终返回时间序列。",
      ),
    limit: z
      .number()
      .int()
    ...
    aggregation: z
      ...
      .optional()
      .describe(
        "聚合方式。日志/链路 timeseries 只使用 count 或 rate；指标默认 avg。",
      ),
    groupBy: z
      ...
      .default([])
      .describe(
        "timeseries 分组字段，例如 service.name、severity_text、host.name 或 consumer_group。",
      ),
    stepSeconds: z
      .number()
      .int()
    ...
    operation: z
      ...
      .optional()
      .describe(
        "仅用于 traces：按 Span 操作名 contains 过滤，例如 POST /orders。",
      ),
    errorOnly: z
      .boolean()
      .optional()
    ...
    metricName: z
      ...
      .optional()
      .describe(
        "仅用于 metrics 且必填，例如 system.filesystem.utilization 或 kafka.consumer.group.lag。",
      ),
    metricType: z
  ...
  .superRefine((input, context) => {
    ...
    if (input.signal === "metrics" && input.aggregation === "count") {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "指标查询不支持 count；请使用 avg、sum、min、max、分位数或 rate",
        path: ["aggregation"],
      });
    }
  })
...
export type SignalQueryInput = z.infer<typeof signalQueryInputSchema>;

type SignalName = SignalQueryInput["signal"];
type AttributeScope = z.infer<typeof attributeScopeSchema>;
type AttributeDataType = z.infer<typeof attributeDataTypeSchema>;
...
const columns: Record<SignalName, Set<string>> = {
  logs: new Set([
    "timestamp",
    "id",
    "trace_id",
...
const fieldAliases: Record<"logs" | "traces", Record<string, string>> = {
  logs: {
...
function attributeKey(
  ...
  if (scope === "auto") {
    if (columns[signal].has(alias)) scope = "column";
    else if (alias === "service.name") scope = "resource";
    else if (
      alias === "scry.debug.case_id" ||
      alias.startsWith("deployment.") ||
      alias.startsWith("host.")
    )
      scope = "resource";
    else scope = "attribute";
  }
  return {
    key: alias,
    dataType: reference.dataType || inferDataType(value),
    type:
      scope === "resource" ? "resource" : scope === "attribute" ? "tag" : "",
...
function autoStepSeconds(lookbackMinutes: number): number {
  return Math.max(5, Math.min(3600, Math.ceil((lookbackMinutes * 60) / 120)));
}

function baseFilters(input: SignalQueryInput): Array<Record<string, unknown>> {
  ...
  if (input.serviceNames.length) {
    const scope: AttributeScope =
      input.signal === "metrics" ? "attribute" : "resource";
    items.push(
      filterItem(
        input.signal,
        "service.name",
...
function logFilters(input: SignalQueryInput) {
...
function traceFilters(input: SignalQueryInput) {
  const items = baseFilters(input);
  if (input.operation)
    items.push(
      filterItem(
  ...
  if (input.minDurationMs !== undefined) {
    items.push(
      filterItem(
        "traces",
        "durationNano",
        Math.round(input.minDurationMs * 1_000_000),
        ">=",
        "column",
        "int64",
      ),
...
function groupByFields(signal: SignalName, values: AttributeReference[]) {
  return values.map((value) => attributeKey(signal, value));
}

function recordsQuery(input: SignalQueryInput) {
  const filters =
    input.signal === "logs" ? logFilters(input) : traceFilters(input);
  return {
...
function signalTimeseriesQuery(input: SignalQueryInput) {
  const filters =
    input.signal === "logs" ? logFilters(input) : traceFilters(input);
  return {
    panelType: "graph",
...
function metricQuery(input: SignalQueryInput) {
  ...
  return {
    ...
    builder: {
      ...
      aggregateAttribute: {
        key: input.metricName as string,
        dataType: "float64",
