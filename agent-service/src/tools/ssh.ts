...
import type { SSHHost, SSHHostSecret } from '../types.js';
...
export type SSHReadonlyCommandId = keyof typeof SSH_READONLY_COMMANDS;

export const SSH_READONLY_COMMAND_LABELS: Record<SSHReadonlyCommandId, string> = {
...
const simulatedOutputByCommand: Record<string, string> = {
...
export function simulateSSHCommand(command: string): SSHExecutionResult {
...
function connectionConfig(host: SSHHost & SSHHostSecret): ConnectConfig {
  return {
    host: host.hostname,
    port: host.port,
    ...
    hostVerifier: (key: Buffer) => normalizedFingerprint(fingerprint(key)) === normalizedFingerprint(host.hostKeyFingerprint),
    readyTimeout: 15_000,
...
export async function probeSSHHostKey(hostname: string, port: number): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    client
      .once('error', (error) => {
        if (!settled) finish(new Error(`无法获取 SSH 主机指纹：${error.message}`));
      })
      .connect({
        host: hostname,
        port,
...
export async function executeSSHCommand(
  host: SSHHost & SSHHostSecret,
  command: string,
  timeoutSeconds = 30,
  outputLimit = DEFAULT_OUTPUT_LIMIT,
): Promise<SSHExecutionResult> {
  if (host.simulated) return simulateSSHCommand(command);
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error?: Error, result?: Omit<SSHExecutionResult, 'durationMs' | 'stdoutBytes' | 'stderrBytes' | 'truncated'>): void => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      client.end();
      if (error) reject(error);
        ...
        ...(result as Omit<SSHExecutionResult, 'durationMs' | 'stdoutBytes' | 'stderrBytes' | 'truncated'>),
    ...
    client
      .once('ready', () => {
        client.exec(command, { pty: false }, (error, stream) => {
          ...
          stream.once('close', (code: number | null, signal: string | null) => finish(undefined, {
            stdout: stdout.toString('utf8'),
            stderr: stderr.toString('utf8'),
            exitCode: code,
            signal,
          }));
          stream.once('error', (streamError: Error) => finish(new Error(`SSH 数据流错误：${streamError.message}`)));
        });
      })
...
export async function testSSHConnection(host: SSHHost & SSHHostSecret): Promise<SSHExecutionResult> {
