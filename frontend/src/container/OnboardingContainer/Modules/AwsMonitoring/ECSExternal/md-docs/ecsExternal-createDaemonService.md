...
Using the template we downloaded and the Scry OpenTelemetry Collector configuration we created, we will create the Daemon Service. This can be done following these steps:
...
aws cloudformation create-stack --stack-name AOCECS-daemon-${CLUSTER_NAME}-${REGION} \
    ...
    ParameterKey=CreateIAMRoles,ParameterValue=True \
        ...
        ParameterKey=ScryConfigPath,ParameterValue=${SIGNOZ_CONFIG_PATH} \
...
To verify that the data is being sent to Scry Cloud, you can go to the dashboard section of Scry and import one of the following dashboards below:

- [instancemetrics.json](https://raw.githubusercontent.com/Scry/dashboards/chore/ecs-dashboards/ecs-infra-metrics/instance-metrics.json)
- [hostmetrics.json](https://raw.githubusercontent.com/Scry/dashboards/main/hostmetrics/hostmetrics.json)

&nbsp;

### Optional Step: Clean Up

In a cloud environment where resources are billed based on usage, cleaning up resources is crucial. This step involves removing the daemon service and any associated resources that were created during the setup process to collect and forward metrics and logs from your ECS infrastructure to Scry. To clean up the daemon service, you can run the following command:
...
Once you follow these steps, you should be able to see your logs and metrics data coming in Scry Cloud. To see data for your traces, click on Continue to next step below.